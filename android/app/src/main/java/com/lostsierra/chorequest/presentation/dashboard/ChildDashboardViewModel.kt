...
import com.lostsierra.chorequest.data.repository.RewardRepository
...
class ChildDashboardViewModel @Inject constructor(
    ...
    val networkStatus: StateFlow<com.lostsierra.chorequest.utils.NetworkStatus> = networkObserver.observeConnectivity()
        .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), com.lostsierra.chorequest.utils.NetworkStatus.Available)
            ...
            try {
                ...
                combine(
                    ...
                    val pendingChores = myChores.filter { 
                        it.status == com.lostsierra.chorequest.domain.models.ChoreStatus.PENDING ||
                        it.status == com.lostsierra.chorequest.domain.models.ChoreStatus.IN_PROGRESS
                    }
                    // Count only chores completed today (check completedAt date)
                    val today = LocalDate.now()
                    val completedToday = myChores.filter { 
                        val isCompleted = it.status == com.lostsierra.chorequest.domain.models.ChoreStatus.COMPLETED ||
                                         it.status == com.lostsierra.chorequest.domain.models.ChoreStatus.VERIFIED
...
sealed class ChildDashboardState {
    ...
    data class Success(
        ...
        val pendingRewards: List<Pair<RewardRedemption, com.lostsierra.chorequest.domain.models.Reward>> = emptyList() // Pending reward redemptions
    ) : ChildDashboardState()
