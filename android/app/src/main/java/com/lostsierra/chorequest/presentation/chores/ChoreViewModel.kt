...
import com.lostsierra.chorequest.data.remote.PhotoUploadRequest
...
import com.lostsierra.chorequest.utils.ImageUtils
            ...
            choreRepository.createChore(newChore).collect { result ->
                when (result) {
                    ...
                    is Result.Error -> {
                        ...
                        val isAuthError = com.lostsierra.chorequest.utils.AuthorizationHelper.isAuthorizationError(result.message)
                        android.util.Log.d("ChoreViewModel", "Is authorization error: $isAuthError")
                        
                        if (isAuthError) {
                            val authUrl = com.lostsierra.chorequest.utils.AuthorizationHelper.extractAuthorizationUrl(result.message)
                                ?: com.lostsierra.chorequest.utils.AuthorizationHelper.getBaseAuthorizationUrl()
                            val errorMsg = com.lostsierra.chorequest.utils.AuthorizationHelper.extractErrorMessage(result.message)
            ...
            try {
                ...
                if (response.isSuccessful && response.body()?.success == true) {
                    ...
                    val photoUrl = if (fileId != null && ownerEmail != null) {
                        ...
                        "${com.lostsierra.chorequest.utils.Constants.APPS_SCRIPT_WEB_APP_URL}?path=photo&fileId=$fileId&ownerEmail=$ownerEmail"
                    } else {
